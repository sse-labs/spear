...
LoopNode::LoopNode(LoopTree *loopTree,
    ProgramGraph *parent,
    AnalysisStrategy::Strategy strategy) : Node(parent, strategy) {
...
double LoopNode::getNodeEnergy(LLVMHandler *handler) {
    ...
    switch (this->strategy) {
        case AnalysisStrategy::WORSTCASE :
                sum = static_cast<double>(this->loopTree->iterations) * sum;
            break;
...
double LoopNode::getMaxEnergy() {
    ...
    maxEnergy = static_cast<double>(this->loopTree->iterations) * maxEnergy;
