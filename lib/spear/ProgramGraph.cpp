...
void ProgramGraph::printNodes(LLVMHandler *handler) {
    ...
    for (auto node : this->nodes) {
        ...
        if (node != nullptr) {
            ...
            llvm::outs() << "(" << dynamic_cast<LoopNode*>(node)->loopTree->iterations << ") "
            << this->getEnergy(handler) << " µJ" << ")";
...
std::string ProgramGraph::printDotRepresentation() {
    ...
    for (int i=0; i < this->nodes.size(); i++) {
        ...
        auto startAddress = reinterpret_cast<uint64_t>(reinterpret_cast<void **>(node));
        ...
        if (loopNodeCandidate != nullptr) {
            ...
            dotStr += node->toString()+
                    "<br/>" + std::to_string(node->energy)+
                    " J> fillcolor=\""+
                    this->getNodeColor(node, maxEng)+
                    "\" style=filled ]\n";
        }
        ...
        for (auto edge : findEdgesStartingAtNode(node)) {
            ...
            auto endAddress = reinterpret_cast<uint64_t>(reinterpret_cast<void **>(edge->end));
            ...
            if (loopStartNodeCandidate != nullptr) {
                ...
                if (loopEndNodeCandidate != nullptr) {
                    dotStr += "invisible_start" + std::to_string(endAddress) +
                        " [lhead=" + "cluster_LOOPNODE_" + std::to_string(startAddress) +
                        " ltail=" + "cluster_LOOPNODE_" + std::to_string(startAddress) +
                        "]\n";
                } else {
