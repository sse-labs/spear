...
const llvm::StoreInst *findDominatingStoreToObject(const llvm::LoadInst *loadInst,
    ...
    for (const llvm::BasicBlock &basicBlock : *function) {
        for (const llvm::Instruction &instruction : basicBlock) {
            ...
            } else if (storeInst->getParent() == loadInst->getParent() && bestStore->getParent() != loadInst->
                       getParent()) {
                bestStore = storeInst;
            }
        }
...
void collectMemRootsFromScalarExpr(const llvm::Value *val, llvm::SmallPtrSetImpl<const llvm::Value *> &OutRoots) {
    ...
    // Remove any casts
    val = LoopBound::Util::stripCasts(val);

    // We need to recursively explore the expression tree to find all memory roots that contribute to given scalar val
    llvm::SmallPtrSet<const llvm::Value *, 32> Visited;
    llvm::SmallVector<const llvm::Value *, 32> Worklist;
    Worklist.push_back(val);

    while (!Worklist.empty()) {
        const llvm::Value *Cur = Worklist.pop_back_val();
        Cur = LoopBound::Util::stripCasts(Cur);
        if (!Cur) {
            continue;
        }
        ...
        if (auto *LI = llvm::dyn_cast<llvm::LoadInst>(Cur)) {
            ...
            Ptr = LoopBound::Util::stripAddr(Ptr);
            if (Ptr) {
                OutRoots.insert(Ptr);
            }
            continue;
        ...
        // Deal with pointer type values
        if (Cur->getType()->isPointerTy()) {
            const llvm::Value *Ptr = LoopBound::Util::stripAddr(Cur);
            if (Ptr) {
                OutRoots.insert(Ptr);
            }
            continue;
...
bool loopIsDependentNested(const LoopParameterDescription &description,
    ...
    collectMemRootsFromScalarExpr(op0, roots0);
    collectMemRootsFromScalarExpr(op1, roots1);

    const bool op0UsesCounter = roots0.contains(counterRoot);
    const bool op1UsesCounter = roots1.contains(counterRoot);
